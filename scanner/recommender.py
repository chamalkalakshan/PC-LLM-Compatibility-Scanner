...
class Recommendation:
    model: LLMModel
    tier: RunTier
    run_mode: str          # "GPU", "CPU", "Partial GPU", "Multi-GPU"
    notes: List[str]
    ollama_cmd: str
...
def _score_model(model: LLMModel, hw: SystemInfo) -> Recommendation:
    ...
    # Partial GPU offload path (some layers on GPU, rest on CPU RAM) ---------
    ...
    # Extra advisory notes ---------------------------------------------------
    if tier not in (RunTier.NO_GO,):
...
def get_recommendations(hw: SystemInfo) -> List[Recommendation]:
    ...
    # Sort: tier (best first), then parameter count descending within tier
